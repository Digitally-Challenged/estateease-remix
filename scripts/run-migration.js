...
async function runMigration() {
  ...
  try {
    ...
    // Read migration file
    const migrationPath = path.join(projectRoot, 'app/db/data-migrations/003-add-business-entity-fields.sql');
    ...
    const verifyIncType = db.prepare(`
      SELECT COUNT(*) as count 
      FROM pragma_table_info('assets') 
      WHERE name = 'incorporation_type'
    `).get();
    
    const verifyState = db.prepare(`
      SELECT COUNT(*) as count 
      FROM pragma_table_info('assets') 
      WHERE name = 'state_of_incorporation'
    `).get();
    
    const verifyEIN = db.prepare(`
      SELECT COUNT(*) as count 
      FROM pragma_table_info('assets') 
      WHERE name = 'ein'
    `).get();
