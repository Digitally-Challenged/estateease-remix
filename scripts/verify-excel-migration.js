...
workbook.SheetNames.forEach(sheetName => {
  ...
  data.forEach((row, index) => {
    ...
    if (rowStr.includes('kathleen') || rowStr.includes('geeslin') || 
        rowStr.includes('kit coleman') || rowStr.includes('kit')) {
    ...
    let assetName = row['Asset'] || row['Account Name'] || row['Description'] || 
                   row['Property'] || row['Name'] || row['Account'] || '';
    let value = row['Value'] || row['Amount'] || row['Balance'] || 
                row['Current Value'] || row['Market Value'] || '0';
...
console.log(`Total rows in Excel (excluding headers/empty): ${verificationResults.totalExcelRows}`);
