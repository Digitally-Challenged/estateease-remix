...
export function calculateNetWorth(assets: Asset[], liabilities: Array<{ amount: number }>): number {
...
export function calculateAssetAllocation(assets: Asset[]): Record<
  string,
  {
    value: number;
    percentage: number;
...
export function calculateTrustValues(trusts: Trust[], assets: Asset[]): Record<string, number> {
...
export function calculateCashFlow(assets: Asset[]): {
  ...
  assets.forEach((asset) => {
    ...
    if ("propertyType" in asset && asset.propertyType === PropertyType.MULTI_FAMILY) {
      ...
      asset.accountType === "INVESTMENT_BROKERAGE"
    ) {
