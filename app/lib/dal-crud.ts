...
export function createAsset(asset: Partial<AnyEnhancedAsset> & { name: string; value: number; category: string }): string {
  ...
  if (category === 'FINANCIAL_ACCOUNT' && asset.details) {
    institutionName = (asset.details as Record<string, unknown>).institutionName as string || null;
    accountType = (asset.details as Record<string, unknown>).accountType as string || null;
    accountNumber = (asset.details as Record<string, unknown>).accountNumber as string || null;
    routingNumber = (asset.details as Record<string, unknown>).routingNumber as string || null;
  }
  ...
  if (category === 'BUSINESS_INTEREST') {
    if (asset.details) {
      incorporationType = (asset.details as Record<string, unknown>).incorporationType as string || null;
      stateOfIncorporation = (asset.details as Record<string, unknown>).stateOfIncorporation as string || null;
    ...
    if (businessAsset.stateOfIncorporation) stateOfIncorporation = businessAsset.stateOfIncorporation;
...
export function updateAsset(assetId: string, updates: Partial<AnyEnhancedAsset>): void {
  ...
  if (updates.category === AssetCategory.BUSINESS_INTEREST) {
    ...
    if (businessUpdates.incorporationType !== undefined && !setClauses.includes('incorporation_type = ?')) {
      setClauses.push('incorporation_type = ?');
      params.push(businessUpdates.incorporationType || null);
    }
    
    if (businessUpdates.stateOfIncorporation !== undefined && !setClauses.includes('state_of_incorporation = ?')) {
      setClauses.push('state_of_incorporation = ?');
...
export function createDocument(document: DocumentCreateInput): string {
  const db = getDatabase();
  ...
  insertStmt.run(
    documentId,
    1, // Default user ID - will need to get from auth context
    document.name,
...
export function logDocumentAccess(documentId: number, userId: number, action: string, ipAddress?: string, userAgent?: string): void {
  const db = getDatabase();
