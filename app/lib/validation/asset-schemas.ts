...
export const assetCategorySchema = z.nativeEnum(AssetCategory, {
  errorMap: () => ({
    message:
      "Invalid asset category. Must be one of: REAL_ESTATE, FINANCIAL_ACCOUNT, INSURANCE_POLICY, BUSINESS_INTEREST, PERSONAL_PROPERTY, VEHICLE, DEBT, OTHER",
  }),
});
...
export const ownershipTypeSchema = z.nativeEnum(OwnershipType, {
  errorMap: () => ({
    message: "Invalid ownership type. Must be one of: INDIVIDUAL, JOINT, TRUST, BUSINESS",
  }),
});
...
export const ownershipSchema = z
  ...
  .refine(
    ...
    {
      message:
        "Trust ID required for trust ownership, Business Entity ID required for business ownership",
      path: ["trustId"],
    },
  );
...
export const financialDataSchema = z.object({
  value: z
    .number()
    .refine((val) => val !== 0, { message: "Value cannot be zero" })
    .refine((val) => Number.isFinite(val), { message: "Value must be a valid number" }),
...
export const assetSchema = z.object({
  name: z
    ...
    .refine((name) => name.trim().length > 0, { message: "Asset name cannot be only whitespace" }),

  category: assetCategorySchema,

  value: z
    .number()
    .refine((val) => val !== 0, { message: "Asset value cannot be zero" })
    .refine((val) => Number.isFinite(val), { message: "Asset value must be a valid number" }),
...
export const assetFormSchema = z.object({
  ...
  category: z
    .string()
    .refine((val) => Object.values(AssetCategory).includes(val as AssetCategory), {
      message: "Invalid asset category",
    }),
  type: z.string().min(1, "Asset type is required"),
  value: z.coerce.number().refine((val) => val !== 0, { message: "Asset value cannot be zero" }),
  description: z.string().max(500, "Description too long").optional().or(z.literal("")),
  notes: z.string().max(1000, "Notes too long").optional().or(z.literal("")),
  ownershipType: z
    .string()
    .refine((val) => Object.values(OwnershipType).includes(val as OwnershipType), {
  ...
  insuranceCompany: z
    .string()
    .max(100, "Insurance company name too long")
    .optional()
    .or(z.literal("")),
  ...
  underlyingPolicies: z
    .string()
    .max(500, "Underlying policies too long")
    .optional()
    .or(z.literal("")),
