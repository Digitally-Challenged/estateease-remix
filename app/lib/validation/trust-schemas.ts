...
const baseTrustSchema = z.object({
  name: z.string()
    ...
    .refine(
      ...
      { message: "Trust name cannot be only whitespace" }
    ),
  ...
  trustees: z.array(trusteeSchema)
  ...
  distributionTerms: z.string()
    .max(1000, "Distribution terms too long")
    .optional(),
 ...
 */
export const trustSchema = baseTrustSchema.refine(
  (data) => {
 ...
 */
export const trustSearchSchema = z.object({
...
export const trustFormSchema = z.object({
  ...
  type: z.string().refine(
    (val) => ['REVOCABLE', 'IRREVOCABLE'].includes(val),
    { message: "Invalid trust type" }
  ),
