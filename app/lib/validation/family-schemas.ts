...
export const contactInfoSchema = z.object({
  ...
  emergencyContact: z.object({
    ...
    relationship: z.string().max(50, "Emergency contact relationship too long").optional().or(z.literal(""))
  }).optional()
});
...
const baseFamilyMemberSchema = z.object({
  ...
  legalRoles: z.array(legalRoleSchema)
    .max(5, "Too many legal roles assigned")
    .default([]),
  ...
  professionalInfo: z.object({
    ...
    specializations: z.array(z.string().max(50, "Specialization too long")).max(10, "Too many specializations").optional(),
...
const baseProfessionalMemberSchema = z.object({
  ...
  specializations: z.array(z.string().max(50, "Specialization too long"))
 ...
 */
export const familyMemberSchema = baseFamilyMemberSchema.refine(
  (data) => {
  ...
  {
    message: "Family members with special needs typically should not serve as executor, trustee, or power of attorney",
    path: ["legalRoles"]
  }
);
...
export const familyMemberFormSchema = z.object({
  ...
  relationship: z.string().refine(
    (val) => ['SPOUSE', 'CHILD', 'PARENT', 'SIBLING', 'GRANDPARENT', 'GRANDCHILD', 'AUNT_UNCLE', 'NIECE_NEPHEW', 'COUSIN', 'IN_LAW', 'STEP_FAMILY', 'OTHER'].includes(val),
...
export const professionalMemberFormSchema = z.object({
  ...
  profession: z.string().refine(
    (val) => ['ESTATE_ATTORNEY', 'TAX_ATTORNEY', 'FAMILY_LAW_ATTORNEY', 'BUSINESS_ATTORNEY', 'CPA', 'FINANCIAL_ADVISOR', 'INVESTMENT_ADVISOR', 'INSURANCE_AGENT', 'TRUST_OFFICER', 'ESTATE_PLANNER', 'REAL_ESTATE_AGENT', 'APPRAISER', 'PHYSICIAN', 'PSYCHIATRIST', 'THERAPIST', 'OTHER'].includes(val),
