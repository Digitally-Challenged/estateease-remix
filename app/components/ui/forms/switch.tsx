...
const Switch = React.forwardRef<HTMLInputElement, SwitchProps>(
  ({ label, helperText, size = "md", id, ...props }, ref) => {
      ...
      <div className="space-y-1">
        <div className="flex items-start">
          <label className="relative inline-flex cursor-pointer items-center">
            ...
            {label && <span className="ml-3 text-sm font-medium text-secondary-700">{label}</span>}
          </label>
        </div>
        {helperText && <p className="ml-14 text-sm text-secondary-500">{helperText}</p>}
      </div>
    );
  },
);
